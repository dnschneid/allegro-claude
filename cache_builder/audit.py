...
def _expand_globs(dir_path: str, includes: list[str], excludes: list[str],
                  files_in_dir: list[str]) -> list[str]:
...
def _rebalance_group_budgets(slice_manifest: dict,
                              files_by_dir: dict[str, list[str]],
                              bytes_by_path: dict[str, int]) -> None:
    ...
    moved = sum(abs(b - g.get("byte_budget", 0))
                for b, g in zip(new_budgets, groups))
    LOG.info("plan: rebalanced budgets across %d groups (total moved: %d bytes; "
             "concept floor applied to %d)",
             len(groups), moved, len(floored_idx))
    for g, b in zip(groups, new_budgets):
        g["byte_budget"] = b


def build_plan(slice_manifest: dict, parts_dir: Path,
               full_manifest: list[dict] | None = None) -> list[dict]:
    ...
    for grp in slice_manifest.get("groups", []):
        ...
        if not files:
            LOG.warning("plan: group %s expanded to 0 files; skipping",
                        grp.get("id"))
            continue
        # Split into <=MAX_FILES_PER_SLICE chunks.
        chunks = [files[i:i + MAX_FILES_PER_SLICE]
        ...
        weighted = [int(group_budget * cs / group_source_bytes)
                    for cs in chunk_source]
        ...
        if total > group_budget and group_budget > sum(floors):
            ...
            budgets = [f + max(0, o - int(excess * o / sum_over))
                       for f, o in zip(floors, over)]
    ...
    LOG.info("plan: %d slices from %d survey groups",
             len(plan), len(slice_manifest.get("groups", [])))
    return plan
...
def run_slices(runner: Runner, plan: list[dict], *, max_parallel: int) -> None:
    ...
    _run_slices_parallel(runner, plan, max_parallel=max_parallel,
                         gap_addenda={})
    ...
    *,
    ...
    def _one(grp: dict):
        ...
        stats = RunStats(
            ...
            duration_s=res.duration_s, tool_uses=res.tool_uses,
        )
        with lock:
    ...
    with ThreadPoolExecutor(max_workers=max_parallel) as pool:
        ...
        MODE_FACTOR = {"concept": 2.0, "reference": 1.0,
                       "verbatim": 0.7, "skip": 0.0}
        def _weight(g):
...
class SliceAudit:
    ...
    def has_gaps(self) -> bool:
        ...
        return bool(
            ...
            or (self.semantic_score is not None and self.semantic_score < 1.0
...
def _audit_structural(grp: dict, signals_in_scope: set[str]) -> SliceAudit:
    ...
    audit = SliceAudit(
        slice_id=grp["id"], part_path=part_path,
    ...
    for name, body in _parse_entries(text):
        ...
        is_stubby = (any(stub in body_lower for stub in STUB_TEXTS)
                     and len(body_compact) < 120)
        if is_stubby:
...
def _scope_signals_to_slices(plan: list[dict],
                              signals: list[str],
                              shadow_root: Path) -> dict[str, set[str]]:
...
_NOISE_SUFFIXES = (".html", ".htm", ".bak", ".old", ".tmp", ".log",
                   ".xml", ".json", ".tgf", ".css", ".js", ".gif",
                   ".png", ".jpg", ".jpeg", ".bmp", ".svg",
                   "_TOC", "_Commands", "TOC", "Commands")
...
def iterate(
    ...
    *,
    ...
    LOG.info("audit: %d signals after noise filter (was %d)",
             len(signals), len(raw_signals))

    for iteration in range(max_iterations + 1):
        LOG.info("audit iteration %d", iteration)
        per_slice_signal_scope = _scope_signals_to_slices(
            plan, signals, shadow_root)
        audits = [_audit_structural(grp, per_slice_signal_scope.get(grp["id"], set()))
                  for grp in plan]
        ...
        merged_text = "".join(
            (Path(grp["part_path"]).read_text(errors="replace")
             if Path(grp["part_path"]).exists() else "")
            for grp in plan)
        global_missing = [s for s in signals if s not in merged_text]
        LOG.info("audit: %d signals missing globally (of %d)",
                 len(global_missing), len(signals))
        ...
        header_idents = set(re.findall(r"^### ([A-Za-z_][A-Za-z0-9_]+)\b",
                                        merged_text, re.MULTILINE))
        ...
        if missing_idents_by_dir:
            ...
            LOG.info("audit: %d significant identifiers missing as `### ` "
                     "headers (across %d dirs)",
                     n_total, len(missing_idents_by_dir))
        ...
        total_size = sum(
            ...
            if Path(grp["part_path"]).exists() else 0
            for grp in plan)
        ...
        if total_size > tightening_threshold:
            ...
            LOG.warning("audit: cache total %d bytes approaches hard cap %d "
                        "(%.2fx of %d threshold); tightening per-slice budgets",
                        total_size, hard_cap, global_oversize_ratio,
        ...
        biggest = max(ref_slices, key=lambda g: g.get("byte_budget", 0)) if ref_slices else None
        ...
        for d, idents in missing_idents_by_dir.items():
            owners = [g for g in plan if g["dir"] == d
                      and g.get("mode") == "reference"]
            if not owners:
                continue
        ...
        if quality_check:
            _run_semantic_check(runner, plan, audits,
                                exhaustive=(quality_check == "exhaustive"))
        ...
        merge_mod.run(plan, merge_to, install_root,
                      hard_cap=hard_cap, target_bytes=target_bytes,
                      audit_report=report, known_idents=known_idents,
                      enforce_cap=False,
        ...
        if iteration >= max_iterations:
            LOG.warning("audit: %d slices still have gaps after %d iterations",
                        len(gap_slices), max_iterations)
            return
        ...
        for a in gap_slices:
            ...
            if prior:
                ...
                shown = prior[:80]
                more = (f" (and {len(prior) - 80} more)"
                        if len(prior) > 80 else "")
                parts.append(
            ...
            if a.ghost_entries:
                parts.append("Previous attempt had empty/ghost bodies for: "
                             + ", ".join(a.ghost_entries[:30])
                             + ". Either fill them with real content from "
                             "source or drop the headers entirely.")
            if a.stub_entries:
                parts.append("Previous attempt had stub-only entries for: "
                             + ", ".join(a.stub_entries[:30])
                             + ". Either replace with real content from "
                             "source or drop the headers entirely.")
            if a.missing_signals:
                parts.append("These identifiers MUST appear (they exist in "
                             "the source files for this slice but were "
                             "omitted): "
                             + ", ".join(a.missing_signals[:80]))
            if a.semantic_failures:
                parts.append("These entries scored useless/partial; ensure "
        ...
        _run_slices_parallel(runner, re_run, max_parallel=max_parallel,
                             gap_addenda=gap_addenda)
    ...
    def _one(grp: dict):
        ...
        for i in range(0, len(sample), batch_size):
            ...
            usr = prompts_mod.QUALITY_USER_TMPL.format(
                slice_id=grp["id"], mode=grp["mode"],
                n=len(batch), entries=entries_text,
            )
            res = runner.run(
                prompt=usr,
            ...
            for line in res.last_assistant_text.splitlines():
                ...
                if not line:
                    continue
                m = re.match(r"^(.+?):\s*(useful|partial|useless)\s*$",
                             line, re.IGNORECASE)
                if not m:
                    continue
        ...
        if total:
            ...
            with lock:
                ...
                LOG.info("qcheck: %s useful=%d partial=%d useless=%d (%.2f)",
                         grp["id"], useful, partial, useless, score)
